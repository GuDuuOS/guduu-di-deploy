...
# Introduction to the codebase of ai-service

## Table of Contents

- [Purpose](#purpose)
- [Environment Setup and Start ai-service Locally](#environment-setup-and-start-ai-service-locally)
...
This document aims to dive deep to the implementation details of ai-service. We have two goals in mind while writing the document:
1. You will be more knowledgeable about how ai-service works under the hood.
2. You will be more confident on what part of codebase is needed for adjustment if you would like to be Guduu DI's contributor.

## Environment Setup and Start ai-service Locally

If you haven't setup the environment or don't know how to run ai-service locally, please refer to the [document](../README.md#setup-for-local-development) here first.

## Codebase Introduction

ai-service is basically an AI service which provides REST api endpoints for access. There are 4 main concepts to ai-service: `API endpoints`, `Services`, `Pipelines` and `Providers`.
...
- The entry point of ai-service is located at [`ai-service/src/__main__.py`](../src/__main__.py)
...
- The file is located at [`ai-service/src/globals.py`](../src/globals.py)
- You can understand the details of service containers and service metadata here
    - service containers(Other services are not supported in UI yet)
        - SemanticsPreparationService: this is responsible for indexing [MDL](https://docs.guduu.local/oss/engine/concept/what_is_mdl) to Qdarnt
    ...
    - service metadata
        - We will record llm's and embedding model's metadata, ai-service version, etc.

### API endpoints

- All business related API endpoints are located at [`ai-service/src/web/v1/routers`](../src/web/v1/routers)
...
- All services are located at [`ai-service/src/web/v1/services`](../src/web/v1/services)

### Pipelines

- All pipelines are located at [`ai-service/src/pipelines`](../src/pipelines)
...
- The abstract class is defined at [`ai-service/src/core/pipeline.py`](../src/core/pipeline.py)

### Providers

- All providers are located at [`ai-service/src/providers`](../src/providers)
- The abstract classes for providers(LLM, embedding model and document store) are defined at [`ai-service/src/core/provider.py`](../src/core/provider.py)
- The abstract class for engine is defined at [`ai-service/src/core/engine.py`](../src/core/engine.py)
