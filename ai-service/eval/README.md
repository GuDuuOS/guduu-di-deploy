...
This document describes the evaluation framework for the Guduu DI AI Service. The evaluation framework is designed to assess the performance of the Guduu DI AI Service based on the following components:
...
4. **Configuration File**: Ensure you have a copy of `config.yaml` located in the `ai-service/eval/` directory.

## Dataset Curation

The dataset curation process is used to prepare the evaluation dataset for the Guduu DI AI Service on evaluation purpose. You can follow the steps below to start the curation app:

- copy `.env.example` to `.env` and fill in the environment variables
- execute the command under the `ai-service` folder: `just curate_eval_data`
...
1. Downloads the specified dataset to:
   ...
   ```txt
   ai-service/tools/dev/etc/<dataset-name>
   ```

2. Prepares and saves evaluation datasets to:

   ```txt
   ai-service/eval/dataset
   ```
...
The prediction process is used to produce the results of the evaluation data using the Guduu DI AI Service. It will create traces and a session on Langfuse to make the results available to the user. You can use the following command to predict the evaluation dataset under the `eval/dataset` directory:
...
The evaluation process is used to assess the prediction results of the Guduu DI AI Service. It compares the prediction results with the ground truth and calculates the evaluation metrics. This process will also add a trace in the same session on Langfuse to make the evaluation results available to the user. You can use the following command to evaluate the prediction results under the `outputs/predictions` directory:

```cli
just eval <prediction-result>
```

Note: If you would like to enable semantics comparison between SQLs by LLM in order to improve the accuracy metric, please fill in Open AI API key in `.env` file in `ai-service/eval` and add `--semantics` to the end of the command like following:
...
- **input**: The user query used as input to the Guduu DI AI Service (e.g., "What is the total number of COVID-19 cases in the US?").
